...
import { PROBABILITY_STATISTICS_DATA } from "../data/probStatsData";
...
export default function ProbStatsTopic() {
  ...
  // Find active topic based on current URL path
  const activeTopic = rawData.find((item) => {
    const title = (item.title || item.topic || item.name || "").toLowerCase();
    if (currentPath.includes("descriptive-statistics")) {
      return title.includes("descriptive") || title.includes("statistics");
    }
    if (currentPath.includes("basic-probability")) {
      return title.includes("probability") || title.includes("basic") || title.includes("bayes");
    }
    return false;
  }) || rawData[0] || {};

  // Extract sections safely
  const questions = activeTopic.questions || activeTopic.mcqs || activeTopic.problems || [];
